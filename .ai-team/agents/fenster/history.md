...
- **PR #57 merge conflict resolution (squad/ralph-watch, 2026-02-22).** PR from spboyer's fork adding `squad watch` command (Ralph local watchdog for persistent polling) had 103 commits behind `origin/dev` after v0.5.0 landed. Conflicts in 10 files: `index.js` (watch command vs v0.5.0 infrastructure), `squad.agent.md` (Ralph references vs .squad/ path updates), `squad-main-guard.yml` (both copies), `.gitignore`, `CONTRIBUTING.md`, docs assets/build. Resolution strategy: preserve BOTH feature sets. Key integration: updated watch command to use `detectSquadDir()` instead of hardcoded `.ai-team` paths for .squad/ migration compatibility. Help text merged to show both `watch` and `scrub-emails` commands. All 64 tests passed post-resolution. Pushed to spboyer's fork; PR status changed from CONFLICTING to MERGEABLE. Critical lesson: when a feature branch diverges from dev by 100+ commits spanning a major version (v0.4.0 → v0.5.0 with directory rename, email scrubbing, project type detection, identity layer), conflicts are structural — delegating to a general-purpose agent for systematic merge was more reliable than manual line-by-line resolution.
